...
#define Auto_Set_Id_Init  [](shared_ptr<Device_class> Device, YAML::Node *Node) -> int\
                                {\
...
#define Auto_Set_Id_CallBack_F    [](shared_ptr<Device_class> Device, u8 *Msg) -> int\
                                        {\
                                            return ((Auto_Set_Id *)Device->Device_Private_Class)->Top_Frame_Analyze(Msg);\
                                        }\
...
public:
    ...
    u16  End_Id = 0;
    int deviceCount;
    shared_ptr<Device_class> Device_this;
