...
#define Motor_Interpolation_Device_Init  [](shared_ptr<Device_class> Device, YAML::Node *Node) -> int\
                                        {\
...
#define Motor_Interpolation_Device_CallBack_F    [](shared_ptr<Device_class> Device, u8 *Msg) -> int\
                                                {\
                                                    return 0;\
                                                }\
...
public:
    ...
    int Init(const shared_ptr<Device_class>& Device, Robot_Hardware *Robot, float ACC, float KP, float KD);

    void To_Buff(const string& Name, float Pos, float max_vel, float torque, float Kp, float Kd) const;

    int INT_GO() const;

    static int Get_Data_From_Yaml_And_Init(const shared_ptr<Device_class>& Device, const YAML::Node& One_Node);
};
