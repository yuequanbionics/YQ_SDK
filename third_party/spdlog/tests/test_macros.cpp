...
TEST_CASE("debug and trace w/o format string", "[macros]") {
    ...
    REQUIRE(ends_with(file_contents(TEST_FILENAME),
                      my_spdlog::fmt_lib::format("Test message 2{}", default_eol)));
    REQUIRE(count_lines(TEST_FILENAME) == 1);

    auto orig_default_logger = my_spdlog::default_logger();
    my_spdlog::set_default_logger(logger);
    ...
    REQUIRE(ends_with(file_contents(TEST_FILENAME),
                      my_spdlog::fmt_lib::format("Test message 4{}", default_eol)));
    my_spdlog::set_default_logger(std::move(orig_default_logger));
}
...
TEST_CASE("pass logger pointer", "[macros]") {
    auto logger = my_spdlog::create<my_spdlog::sinks::null_sink_mt>("refmacro");
    auto &ref = *logger;
