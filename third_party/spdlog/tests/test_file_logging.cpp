...
TEST_CASE("simple_file_logger", "[simple_logger]") {
    prepare_logdir();
    my_spdlog::filename_t filename = SPDLOG_FILENAME_T(SIMPLE_LOG);

    auto logger = my_spdlog::create<my_spdlog::sinks::basic_file_sink_mt>("logger", filename);
    ...
    REQUIRE(file_contents(SIMPLE_LOG) ==
            my_spdlog::fmt_lib::format("Test message 1{}Test message 2{}", default_eol, default_eol));
}
...
TEST_CASE("flush_on", "[flush_on]") {
    ...
    logger->flush_on(my_spdlog::level::info);
    ...
    using my_spdlog::details::os::default_eol;
    REQUIRE(file_contents(SIMPLE_LOG) ==
            my_spdlog::fmt_lib::format("Should not be flushed{}Test message 1{}Test message 2{}",
                                    default_eol, default_eol, default_eol));
}

TEST_CASE("simple_file_logger", "[truncate]") {
    prepare_logdir();
    const my_spdlog::filename_t filename = SPDLOG_FILENAME_T(SIMPLE_LOG);
    const bool truncate = true;
    const auto sink = std::make_shared<my_spdlog::sinks::basic_file_sink_mt>(filename, truncate);
    const auto logger = std::make_shared<my_spdlog::logger>("simple_file_logger", sink);
...
TEST_CASE("rotating_file_logger1", "[rotating_logger]") {
    ...
    my_spdlog::filename_t basename = SPDLOG_FILENAME_T(ROTATING_LOG);
    auto logger = my_spdlog::rotating_logger_mt("logger", basename, max_size, 0);
...
TEST_CASE("rotating_file_logger3", "[rotating_logger]") {
    ...
    size_t max_size = 0;
    my_spdlog::filename_t basename = SPDLOG_FILENAME_T(ROTATING_LOG);
    REQUIRE_THROWS_AS(my_spdlog::rotating_logger_mt("logger", basename, max_size, 0),
                      my_spdlog::spdlog_ex);
}
...
TEST_CASE("rotating_file_logger4", "[rotating_logger]") {
    ...
    size_t max_size = 1024 * 10;
    my_spdlog::filename_t basename = SPDLOG_FILENAME_T(ROTATING_LOG);
    auto sink = std::make_shared<my_spdlog::sinks::rotating_file_sink_st>(basename, max_size, 2);
    auto logger = std::make_shared<my_spdlog::logger>("rotating_sink_logger", sink);
...
TEST_CASE("rotating_file_logger5", "[rotating_logger]") {
    ...
    my_spdlog::filename_t basename = SPDLOG_FILENAME_T(ROTATING_LOG);
    auto sink =
        std::make_shared<my_spdlog::sinks::rotating_file_sink_st>(basename, max_size, max_files);
    auto logger = std::make_shared<my_spdlog::logger>("rotating_sink_logger", sink);
    logger->set_pattern("%v");
