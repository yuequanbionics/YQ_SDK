...
TEST_CASE("custom_callback_logger", "[custom_callback_logger]") {
    ...
    my_spdlog::pattern_formatter formatter;
    auto callback_logger =
        std::make_shared<my_spdlog::sinks::callback_sink_st>([&](const my_spdlog::details::log_msg &msg) {
            my_spdlog::memory_buf_t formatted;
            formatter.format(msg, formatted);
            auto eol_len = strlen(my_spdlog::details::os::default_eol);
            using diff_t =
    ...
    my_spdlog::logger logger("test-callback", {callback_logger, test_sink});
