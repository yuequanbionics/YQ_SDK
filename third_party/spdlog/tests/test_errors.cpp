...
class failing_sink : public my_spdlog::sinks::base_sink<std::mutex> {
protected:
    void sink_it_(const my_spdlog::details::log_msg &) final {
...
TEST_CASE("default_error_handler", "[errors]") {
    ...
    auto logger = my_spdlog::create<my_spdlog::sinks::basic_file_sink_mt>("test-error", filename, true);
    logger->set_pattern("%v");
    logger->info(SPDLOG_FMT_RUNTIME("Test message {} {}"), 1);
    logger->info("Test message {}", 2);
    logger->flush();
    using my_spdlog::details::os::default_eol;
    REQUIRE(file_contents(SIMPLE_LOG) == my_spdlog::fmt_lib::format("Test message 2{}", default_eol));
...
TEST_CASE("custom_error_handler", "[errors]") {
    prepare_logdir();
    my_spdlog::filename_t filename = SPDLOG_FILENAME_T(SIMPLE_LOG);
    auto logger = my_spdlog::create<my_spdlog::sinks::basic_file_sink_mt>("logger", filename, true);
...
TEST_CASE("default_error_handler2", "[errors]") {
    my_spdlog::drop_all();
    auto logger = my_spdlog::create<failing_sink>("failed_logger");
    logger->set_error_handler([=](const std::string &) { throw custom_ex(); });
...
TEST_CASE("async_error_handler", "[errors]") {
    ...
    my_spdlog::filename_t filename = SPDLOG_FILENAME_T(SIMPLE_ASYNC_LOG);
    {
        my_spdlog::init_thread_pool(128, 1);
        auto logger =
            my_spdlog::create_async<my_spdlog::sinks::basic_file_sink_mt>("logger", filename, true);
        ...
        logger->info("Good message #2");
        my_spdlog::drop("logger");  // force logger to drain the queue and shutdown
    }
    my_spdlog::init_thread_pool(128, 1);
...
TEST_CASE("async_error_handler2", "[errors]") {
    ...
    {
        my_spdlog::details::os::create_dir(SPDLOG_FILENAME_T("test_logs"));
        my_spdlog::init_thread_pool(128, 1);
        auto logger = my_spdlog::create_async<failing_sink>("failed_logger");
        logger->set_error_handler([=](const std::string &) {
        ...
        my_spdlog::drop("failed_logger");  // force logger to drain the queue and shutdown
    }
